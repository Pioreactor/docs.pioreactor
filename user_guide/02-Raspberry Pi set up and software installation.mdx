...
1.  A Raspberry Pi (our list of preferred Raspberry Pis is [here](https://pioreactor.com/pages/faqs-and-how-do-is#what-raspberrypi-hardware-can-i-use-i-see-raspberrypi-1-2-3-a-b--zero---its-confusing))
...
1. Download the [Pioreactor Leader image](https://github.com/pioreactor/pioreactor/releases/latest/download/leader_worker.img.zip): this has all the required software preinstalled on it, including the latest version of Raspberry Pi OS and Pioreactor.
2. Download the [Raspberry Pi Imager](https://www.raspberrypi.org/software/) for your operating system, install it and open the Raspberry Pi Imager.
   1. Hold down the key sequence "ctrl-shift-x" to bring up the settings menu.
  ![](/img/user_guide/ctrlshiftx.png)
    2. Set hostname as the desired name of this Pioreactor. Some names that we use for our first Pioreactor: ` pioreactor1`, or `leader`, or `p1`.
    2. Check "Enable SSH" and add a password
...
3.  Click "Choose OS". Click on "Use custom" towards the bottom:
    ![](/img/user_guide/use_custom_image.png)
    Select the downloaded Pioreactor image from step 1.

    ![](/img/user_guide/select_new_image.png)

1.  Put your microSD card into your computer. This is what you'll be using for 'storage'.
2.  Once your microSD card is in your computer, select it under "Choose Storage".
4.  Click "Write".
4.  Once writing is complete complete, remove the microSD card, and place it into the Raspberry Pi.
5.  Attach your Pioreactor HAT to your Raspberry Pi.
6.  Ensure the two pieces fit snugly together.
7.  Plug the power cord into your Raspberry Pi in. LEDs onboard the Raspberry Pi should start to light up.
8. After a few moments, your Pioreactor will flash a blue LED and now is ready to go. Navigate to [http://pioreactor.local](http://pioreactor.local) in a web browser to visit the web interface. (Not working? See [Troubleshooting](#Troubleshooting) below).
