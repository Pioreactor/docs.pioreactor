...
1.  A Raspberry Pi (our list of preferred Raspberry Pis is [here](/user_guide/Common%20questions#what-raspberry-pi-hardware-can-i-use-i-see-raspberry-pi-1-2-3-4-a-b--zero---its-confusing))
...
2.  Click "Choose OS". Click on "Use custom" towards the bottom:
    ![Select the "Use custom" option](/img/user_guide/use_custom_image.png)
    Select the downloaded Pioreactor image from step 1.
    ![In the local computers direction, select the downloaded image](/img/user_guide/select_new_image.png)
2. Connect your microSD card to your computer. Once your microSD card connected to your computer, select it under "Choose Storage".
4. Click the settings menu (Gear button in the bottom left).
  ![Settings menu](/img/user_guide/ctrlshiftx.png)
    ...
    2. Check "Enable SSH".
    3. (Optional) Add a password, default is `raspberry` if not provided.
    3. Check "Configure wifi" and enter your network name into SSID, wifi password, and country.
    4. (Optional) Change timezone to your local timezone.
    5. Click "Save"

4.  Click "Write".
4.  Once writing is complete complete, remove the microSD card, and place it into the Raspberry Pi.
5.  (Optional) If not already done so, attach your Pioreactor HAT to your Raspberry Pi. Ensure the two pieces fit snugly together.
6.  Plug the power cord into your Raspberry Pi in. LEDs onboard the Raspberry Pi should start to light up.
7.  After a few moments, your Pioreactor will flash a blue LED and now is ready to go. Navigate to [http://pioreactor.local](http://pioreactor.local) in a web browser to visit the web interface. (Not working? See [Troubleshooting](/user_guide/Getting%20started/Raspberry%20Pi%20set%20up%20and%20software%20installation#troubleshooting) below).
...
Follow the same instructions above, but use a [Pioreactor Worker image](https://github.com/pioreactor/pioreactor/releases/latest/download/leader_worker.img.zip) instead. **Choose a hostname that isn't currently being used**.
...
Provide the name of the new worker Pioreactor:

![](https://user-images.githubusercontent.com/884032/104633581-586e9c00-566d-11eb-9ea9-cffbf55d7303.png)

After a few minutes, the new Pioreactor should appear on the _Pioreactors_ and _Experiment Overview_ page.
...
On a successful start, the Pioreactor will flash its blue LED for about 4 seconds. If you observe the blue LED flashing for longer, something may be wrong. You can diagnose the problem by [counting the number of flashes observed](/user_guide/Advanced/Flashing%20blue%20led)
