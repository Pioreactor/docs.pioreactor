...
#### When I click an action in the UI, I don't see any response
 ...
 - Is there a green icon in the top-left of the Pioreactor card? If not, or it's grey, either the Pioreactor is off, or the `monitor` job is not running. Try power-cycling the Pioreactor.
