...
sidebar_label: Self-test reference
description: What each self-test check means and how to troubleshoot failures.
slug: /self-test-reference
---
...
The test will also report any valid IR LED and photodiode relationships in the dialog box. For example, `IR -> REF` means that the REF photodiode has a linear relationship with the IR LED. Similarly, `IR -> 45` means the the photodiode in the 45 deg pocket has a linear relationship with the IR LED. The absence of a relationship here can tell you about what might be wrong.
