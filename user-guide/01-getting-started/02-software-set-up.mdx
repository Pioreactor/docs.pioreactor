...
4. Click the settings menu (Gear button in the bottom left).
  ![Settings menu](/img/user-guide/ctrlshiftx.png)
    ...
    3. Check "Configure wifi" and enter your network name into SSID, wifi password, and country. (Alternative: instead of trying to use a "restrictive" network, like at a university that may not allow Raspberry Pi's, [here's a built-in solution for creating a local access point](/user-guide/local-access-point)).
