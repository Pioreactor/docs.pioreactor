...
10. **If you're using a Pioreactor 20ml v1.0 (sold prior to May 2024) edit your config.ini to**
    ```
    [pioreactor]
    model=pioreactor_20ml
    version=1.0
    ```
    You can find this in the UI -> Configuration -> select your Pioreactor's config.ini from the dropdown, edit, and save. Finally **reboot your Pioreactor**.
...
2. Follow the same instructions above to write the image to an microSD card. All the settings are the same, including *username* and *password*, **but choose a new hostname**. See note below:
   ...
   If you plan to have many Pioreactors, use a leading zero in any numbers, like `pio01`, or `worker01`, etc.
   :::
...
2. If using a local-access-point on your leader Pioreactor, use the wifi ssid `pioreactor` and password `raspberry`.
3. After the image is written to the microSD card, place the microSD card into the Raspberry Pi and power it on.
4. The Pioreactor's blue LED will light up and stay on, indicating that it's ready to be connected. Move onto the next step.
