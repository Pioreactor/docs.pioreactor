...
![a red badge is beside the Update link](/img/user-guide/03-extending-your-pioreactor/20-updating-software/badge.png)
...
![highlight of the webpage showing what to select to update](/img/user-guide/03-extending-your-pioreactor/20-updating-software/click_update.png)

Updating from the UI will update all the Pioreactors in your cluster.

:::note

A cluster can be on mixed software versions temporarily while a rolling update is in progress. Version skew may expose incompatible features or APIs, so we recommend completing the update across the cluster instead of operating mixed versions long-term.
...
### Method 3: Update using a zip file from a USB drive

1. Download the release you wish to update to: <ButtonDownloadVersionedReleaseZip/>

2. Copy the `release_*.zip` file to the root of a USB drive.
...
4. In the UI, open **Updates**. In the dropdown at the top right, select **Update from USB**.
...
5. Choose the release archive and the Pioreactors to update, then click **Update**.

:::info

Official release archives are verified before update scripts run. If an archive is missing its signed manifest, has been modified, or is not a Pioreactor release archive, the update will stop before installation.

:::


### Method 4: Update using a zip file over scp or sftp
...
3. Enter the following credentials in the top-right:
   ...
   ![filezilla UI connecting to Pioreactor's Raspberry Pi](/img/user-guide/03-extending-your-pioreactor/20-updating-software/connect_via_filezilla.png)
...
5. On the left-hand side is your computer's file system. Find the `release_xx.xx.xx.zip` file you downloaded earlier, and double click it to upload to the Raspberry Pi (that filesystem is on the right).
   ![filezilla UI uploading file to Pioreactor](/img/user-guide/03-extending-your-pioreactor/20-updating-software/upload_file_via_filezilla.png)
6. You should see a successfully uploaded message, and the file should be present on the right-hand side now. You can close FileZilla.
  ![filezilla UI successfully uploaded file to Pioreactor](/img/user-guide/03-extending-your-pioreactor/20-updating-software/successuful_upload.png)
